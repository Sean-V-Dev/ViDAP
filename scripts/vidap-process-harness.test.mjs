...
function validFixtureText() {
  ...
  return JSON.stringify({
    ...fixture,
    byteSize: Buffer.byteLength(initial, "utf8") + 2,
  });
}
...
describe("vidap process harness inputs", () => {
  it("rejects an unsupported mode without starting a process", () => {
    expect(() => validateMode("invalid")).toThrow(
      'must be "launch" or "smoke"',
    );
  });

  it("rejects non-loopback and alternate readiness targets", () => {
    expect(() =>
      validateLoopbackStatusUrl("http://localhost:8000/api/status", 8000),
    ).toThrow("prescribed loopback");
    expect(() =>
      validateLoopbackStatusUrl("https://127.0.0.1:8000/api/status", 8000),
    ).toThrow("prescribed loopback");
  });

  it("accepts only complete bounded fixture metadata", () => {
    expect(parseFixtureText(validFixtureText()).expectedStatus).toEqual(
      FOUNDATION_STATUS,
    );
    expect(() => parseFixtureText("{not-json")).toThrow("valid UTF-8 JSON");
    expect(() =>
      parseFixtureText(`${validFixtureText()}${" ".repeat(1_025)}`),
    ).toThrow("at or below 1024 bytes");
  });
  ...
  it("keeps failure diagnostics bounded and actionable without a log file", () => {
    ...
    expect(
      failureMessage("Python readiness", "Use npm.cmd run setup.", diagnostics),
    ).toContain("Use npm.cmd run setup.");
  });
});
